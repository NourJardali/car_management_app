...
class ExpertSignUpPage extends StatefulWidget {
  @override
  _ExpertSignUpPageState createState() => _ExpertSignUpPageState();
}
...
class _ExpertSignUpPageState extends State<ExpertSignUpPage> {
      ...
      child: Form(
        ...
        child: Column(
          children: <Widget>[
            ...
            SizedBox(height: _height / 60.0),
            garageNameTextFormField(),
            SizedBox(height: _height / 60.0),
            expertTypeDropDownMenu(),
            SizedBox(height: _height / 60.0),
            expertDescTextFormField(),
          ],
        ),
      ),
    );
  ...
  Widget locationTextFormField() {
    return Material(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          prefixIcon:
              Icon(Icons.location_city, color: Colors.orange[200], size: 20),
          hintText: 'Location',
          border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(30.0),
              borderSide: BorderSide.none),
  ...
  Widget garageNameTextFormField() {
    return Material(
      borderRadius: BorderRadius.circular(30.0),
      elevation: _large ? 12 : (_medium ? 10 : 8),
      child: TextFormField(
        onSaved: (val) => _garagename = val,
        keyboardType: TextInputType.text,
        cursorColor: Colors.orange[200],
        decoration: InputDecoration(
          prefixIcon:
  ...
  Widget expertDescTextFormField() {
    return Material(
      borderRadius: BorderRadius.circular(30.0),
      elevation: _large ? 12 : (_medium ? 10 : 8),
      child: TextFormField(
    ...
    if (form.validate()) {
      setState(() async {
        ...
        if (checkBoxValue == false) {
          _showAlertDialog('Status', 'Accept rights');
        } else {
          await authHandler
            ...
            if (user != null) {
              var user = CarExpert(
                  ...
                  location: _location);
              databaseHelper.insertCarExpert(user);
              Navigator.push(
                context,
              ...
              _showAlertDialog(
                  'Sign up failed', 'Email Address Already Exists');
            }
